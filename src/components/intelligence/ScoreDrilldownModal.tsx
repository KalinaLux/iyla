...
import { getIylaScoreHistory } from '../../lib/intelligence';
...
const STATUS_STYLE: Record<string, { bar: string; label: string }> = {
  ...
  watch: { bar: 'bg-rose-400', label: 'text-rose-500' },
};

export default function ScoreDrilldownModal({ open, onClose, score }: Props) {
  const history = getIylaScoreHistory().slice(-30);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Your iyla Score">
      <div className="space-y-5 px-1 pb-1">
        {/* Summary */}
        <div className="bg-gradient-to-br from-warm-800 to-warm-900 text-white rounded-3xl p-6">
          <div className="flex items-end justify-between">
            ...
            <div className="text-right text-xs text-white/70 space-y-0.5">
              <div className="flex items-center gap-1 justify-end">
                ...
                <span className="font-medium">{score.delta7Day > 0 ? '+' : ''}{score.delta7Day} this week</span>
              </div>
              <div>{score.delta30Day > 0 ? '+' : ''}{score.delta30Day} over 30 days</div>
            </div>
          </div>
          {sparkPath && (
                ...
                <div key={d.id} className="p-4 bg-white rounded-2xl border border-warm-100">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      ...
                      <p className={`text-[11px] font-medium capitalize ${st.label}`}>{d.status}</p>
                    </div>
                  ...
                  <div className="h-1.5 bg-warm-100 rounded-full overflow-hidden">
                    <div className={`h-full ${st.bar} rounded-full transition-all duration-700`} style={{ width: `${d.score}%` }} />
                  </div>
                    ...
                    <div className="mt-3 space-y-1.5">
                      {d.contributingSignals.slice(0, 4).map((s, i) => (
                        <div key={i} className="flex items-center justify-between text-[11px]">
                          <span className="text-warm-500 truncate">{s.name}</span>
        ...
        <div className="text-[11px] text-warm-400 text-center pt-1">
          Data completeness: {Math.round(score.dataCompleteness * 100)}% · Updated {new Date(score.computedAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}
        </div>
      </div>
    </Modal>
  );
